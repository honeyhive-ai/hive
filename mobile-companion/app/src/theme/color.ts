...
//
// The desktop shell builds ~20 tokens in CSS with `color-mix(in srgb, …)` and
// `oklch(from var(--x) L c h)`. React Native has neither custom properties nor
// those functions, so the same derivations run here in JS at theme-resolve time
// and the components consume flat, fully-resolved colour strings.
//
...
//  2. `oklch(from X L c h)` with L replaced routinely lands outside sRGB —
//     replacing lightness while holding chroma is exactly the move that leaves
//     the gamut. What happens next is a *measured* fact, not a spec question:
//     see `GamutStrategy` below. The short version is that the desktop app
//     renders in Chromium, Chromium clips per channel, and parity with the
//     desktop is the requirement — so clipping is the default here.
...
/// Parse the CSS colour notations the desktop palettes actually use: `rgb()`,
/// `rgba()`, `#rgb` / `#rrggbb` / `#rrggbbaa`, and the `transparent` keyword.
/// Throws on anything else rather than silently returning black — a palette
/// typo should fail loudly in a test, not render as a plausible dark colour.
...
/// Serialise for React Native's style props. Channels round to integers; alpha
/// keeps three decimals (enough that a 6% hover tint survives the round-trip).
...
/// little. **This is what Chromium does**, which makes it what the desktop
/// shell does: Hive's desktop is Tauri, and Tauri renders on WebView2 —
/// Chromium — on Windows and on WebKitGTK on Linux.
///
...
/// to hold hue. Safari/WebKit does this.
///
/// The default is `"clip"`, and the reason is worth stating plainly because it
/// is the opposite of what the spec would suggest. This app's job is to look
/// like the desktop app, not to be independently correct. The two strategies
/// genuinely disagree on the shipped palettes — dark `dangerInk` comes out
/// rgb(255,157,155) clipped and rgb(255,173,170) mapped, which reads as a
/// saturated red versus a pink — and every value in the test fixture was
/// measured out of real rendered Chromium pixels, so `"clip"` is the one that
/// matches. `"css4"` is kept, and tested, because it is what WKWebView renders:
/// a macOS desktop build already disagrees with the Windows build on these
/// tokens, and if that is ever reconciled this is the switch to flip.
...
/// `l` is the 0–1 lightness (the desktop tokens spell it as a percentage).
